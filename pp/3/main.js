...
window.onscroll = () => {
    ...
    sections.forEach((sec) => {
        ...
        if (top >= offset && top < offset + height && !sec.classList.contains('active')) {
...
function popup(event) {
    ...
    const imgElements = Array.from(event.target.closest('.prg-mini').querySelectorAll('img'));
    currentImages = imgElements.map(img => img.outerHTML);
    currentIndex = 0;

    mainpop.innerHTML = currentImages[currentIndex];
    subcon.innerHTML = currentImages.join('');
    
    leftbtn.style.display = 'none';
    rightbtn.style.display = currentImages.length > 1 ? 'flex' : 'none';
}
...
function right() {
    if (currentIndex < currentImages.length - 1) {
        currentIndex++;
        mainpop.innerHTML = currentImages[currentIndex];
        rightbtn.style.display = currentIndex === currentImages.length - 1 ? 'none' : 'flex';
        leftbtn.style.display = 'flex';
    }
}
